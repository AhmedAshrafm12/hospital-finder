...
interface FranchiseBookingFormProps {
  factoryId: number;
}

const FranchiseBookingForm = ({ factoryId }: FranchiseBookingFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    submitData.append('franchise', 'franchise'); // Add franchise parameter
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      >
        {isSubmitting ? t('common.submitting') : t('franchise.requestFranchise')}
      </Button>
    </form>
  );
};

export default FranchiseBookingForm; 